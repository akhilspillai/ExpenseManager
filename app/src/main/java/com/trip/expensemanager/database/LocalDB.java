package com.trip.expensemanager.database;
...
public class LocalDB {
    ...
    public String getCountryCode() {
        ...
        SQLiteDatabase database = open();
        cursor = database.query(SQLiteHelper.TABLE_LOGIN, new String[]{
                SQLiteHelper.COLUMN_COUNTRY_CODE}, null, null, null, null, null);
        if (cursor.moveToFirst()) {
            strCountryCode = cursor.getString(0);
        }
    ...
    public boolean deleteLogin(long userId) {
        boolean done = false;
        try {
            open().delete(SQLiteHelper.TABLE_LOGIN, SQLiteHelper.COLUMN_USER_ID + " = ?",
    ...
    public boolean deleteUser(long userId) {
        boolean done = false;
        try {
            open().delete(SQLiteHelper.TABLE_USERS, SQLiteHelper.COLUMN_USER_ID + " = ?",
                    new String[]{String.valueOf(userId)});
            done = true;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean updateTripUsers(long lngTripId, String strUsernames, String strSyncStatus) {
        try {
            SQLiteDatabase database = open();
            ContentValues args = new ContentValues();
            args.put(SQLiteHelper.COLUMN_USERS, strUsernames);
            args.put(SQLiteHelper.COLUMN_IS_SYNCHED, strSyncStatus);
            database.update(SQLiteHelper.TABLE_TRIP, args, SQLiteHelper.COLUMN_TRIP_ID + "=?",
                    new String[]{String.valueOf(lngTripId)});
            return true;
        } catch (Exception e) {
    ...
    public boolean insertPerson(long lngUserId, String strUsername, String strPrefferedName, String strSyncStatus) {
        boolean done = false;
        try {
            SQLiteDatabase database = open();
    ...
    public ArrayList<UserBean> retrieveNotSynchedUsers() {
        ArrayList<UserBean> strArrUsers = new ArrayList<>();
    ...
    public void clearAllTables() {
        try {
            SQLiteDatabase database = open();
            database.delete(SQLiteHelper.TABLE_LOGIN, null, null);
            database.delete(SQLiteHelper.TABLE_TRIP, null, null);
            database.delete(SQLiteHelper.TABLE_EXPENSE, null, null);
            database.delete(SQLiteHelper.TABLE_USERS, null, null);
            database.delete(SQLiteHelper.TABLE_TO_SYNC, null, null);
            database.delete(SQLiteHelper.TABLE_DISTRIBUTION, null, null);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            close();
